...
class TSizeChangedLayoutNotification extends LayoutChangedNotification{
    TSizeChangedLayoutNotification(this.newSize):super();
    Size newSize;
}
...
///    widget tree.
class TSizeChangedLayoutNotifier extends SingleChildRenderObjectWidget {
    /// Creates a [SizeChangedLayoutNotifier] that dispatches layout changed
    /// notifications when [child] changes layout size.
    const TSizeChangedLayoutNotifier({
        Key key,
        Widget child
    }) : super(key: key, child: child);

    @override
    _TRenderSizeChangedWithCallback createRenderObject(BuildContext context) {
        return new _TRenderSizeChangedWithCallback(
            onLayoutChangedCallback: (size) {
                new TSizeChangedLayoutNotification(size).dispatch(context);
            }
        );
    }
}

class _TRenderSizeChangedWithCallback extends RenderProxyBox {
    _TRenderSizeChangedWithCallback({
        RenderBox child,
