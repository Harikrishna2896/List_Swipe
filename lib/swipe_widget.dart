...
class TRightSideButton extends Object{
    TRightSideButton({@required this.name,@required this.onPress, this.backgroudColor:Colors.grey, this.fontColor:Colors.white}){
        assert(name != null);
        assert(onPress != null);
    }
...
class TLeftSlide extends StatefulWidget {
    TLeftSlide({Key key, @required this.buttons, @required this.child, this.backgroundColor:Colors.white}):super(key:key){
        assert(buttons.length <= 3);
    }

    final List<TRightSideButton> buttons;
    final Widget child;
...
class _LeftSlideState extends State<TLeftSlide> {
    ...
    Widget build(BuildContext context) {
        if (childSize == null){
            return new NotificationListener(
                child: new TSizeChangedLayoutNotifier(
                    child: widget.child,
                ),
                onNotification: (TSizeChangedLayoutNotification notification){
        ...
        for (TRightSideButton button in widget.buttons){
            under.add(
                new Container(
